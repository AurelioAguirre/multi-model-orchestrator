...
"""
vLLM Service Routes
API endpoints for vLLM high-performance inference service
"""
...
logger = logging.getLogger("VLLMRoutes")

# Create router
router = APIRouter()

# Global provider instance (will be set by main.py)
provider: VLLMAsyncProvider = None

def get_provider() -> VLLMAsyncProvider:
...
def set_provider(vllm_provider: VLLMAsyncProvider):
...
async def load_model(request: ModelLoadRequest, provider: VLLMAsyncProvider = Depends(get_provider)):
    """Load vLLM model"""
    try:
        ...
        return ModelLoadResponse(
            status="success",
            message="vLLM model loaded successfully",
...
async def unload_model(provider: VLLMAsyncProvider = Depends(get_provider)):
    """Unload vLLM model"""
    try:
        await provider.unload_model()
        return {"success": True, "message": "vLLM model unloaded successfully"}
    except Exception as e:
...
async def create_completion(request: CompletionRequest, provider: VLLMAsyncProvider = Depends(get_provider)):
...
async def create_chat_completion(request: ChatCompletionRequest, provider: VLLMAsyncProvider = Depends(get_provider)):
...
@router.get("/generate")
async def generate(prompt: str, max_tokens: int = 100, provider: VLLMAsyncProvider = Depends(get_provider)):
