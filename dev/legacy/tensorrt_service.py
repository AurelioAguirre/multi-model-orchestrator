#!/usr/bin/env python3
"""
TensorRT-LLM Microservice
FastAPI service for TensorRT-LLM inference
"""
...
from src.providers.tensorrt_provider import TensorRTProvider
...
logger = logging.getLogger("TensorRTService")

# FastAPI app
app = FastAPI(
    title="TensorRT-LLM Microservice",
    description="TensorRT-LLM optimized inference service",
    version="1.0.0"
)
...
async def startup_event():
    ...
    try:
        ...
        logger.info("TensorRT-LLM service initialized successfully")
...
if __name__ == "__main__":
    ...
    port = int(os.getenv("TENSORRT_SERVICE_PORT", 8014))
    
    logger.info(f"Starting TensorRT-LLM service on port {port}")
    
    uvicorn.run(
        "tensorrt_service:app",
        host="0.0.0.0",
        port=port,
        log_level="info",
        reload=False
